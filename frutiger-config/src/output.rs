...
use knuffel::Decode;
use frutiger_ipc::{ConfiguredMode, HSyncPolarity, Transform, VSyncPolarity};
...
pub struct Modeline {
    ...
    pub hsync_polarity: frutiger_ipc::HSyncPolarity,
    /// Vertical sync polarity: "+vsync" or "-vsync".
    pub vsync_polarity: frutiger_ipc::VSyncPolarity,
}
...
pub struct MaxBpc(pub frutiger_ipc::MaxBpc);
...
impl OutputName {
    pub fn from_ipc_output(output: &frutiger_ipc::Output) -> Self {
        Self {
    ...
    fn raw_decode(
        ...
        match &**value {
            knuffel::ast::Literal::Int(ref val) => match u8::try_from(val) {
                Ok(v) => frutiger_ipc::MaxBpc::try_from(v)
                    .map(MaxBpc)
