...
use frutiger::animation::Clock;
use frutiger::layout::{ActivateWindow, AddWindowTarget, LayoutElement as _, Options, SizingMode};
use frutiger::render_helpers::{RenderCtx, RenderTarget};
use frutiger_config::{Color, OutputName, PresetSize};
...
pub struct Layout {
    ...
    clock: Clock,
    layout: frutiger::layout::Layout<TestWindow>,
    start_time: Duration,
    ...
    pub fn new(args: Args) -> Self {
        ...
        let options = Options {
            layout: frutiger_config::Layout {
                focus_ring: frutiger_config::FocusRing {
                    off: true,
                    ..Default::default()
                },
        ...
        let mut layout = frutiger::layout::Layout::with_options(clock.clone(), options);
