...
impl InsertHintElement {
    pub fn new(config: frutiger_config::InsertHint) -> Self {
        Self {
            inner: FocusRing::new(frutiger_config::FocusRing {
                off: config.off,
                width: 0.,
    ...
    pub fn update_config(&mut self, config: frutiger_config::InsertHint) {
        self.inner.update_config(frutiger_config::FocusRing {
            off: config.off,
            width: 0.,
            active_color: config.color,
            inactive_color: config.color,
